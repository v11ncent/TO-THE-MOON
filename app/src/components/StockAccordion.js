...
function StockAccordion(props) {
  ...
  return (
    ...
    <Accordion>
      ...
      {props.stocks.map(({ symbol, occurences }, index) => {
        return (
          <Card key={symbol} border="dark" style={{ width: '100%', backgroundColor: 'white' }} >
            <Card.Header style={{ textAlign: 'center' }} > 
              <Accordion.Toggle as={Button} variant="link" eventKey={index + 1}>
                  <label style={{ margin: 0, textTransform: 'uppercase', textAlign: 'left', cursor: 'pointer' }}>{symbol}</label>
                  <p style={{textAlign: 'right'}}>{ ' Relevance Score: ' + (( occurences / props.totalOccurence) * 100).toFixed(2) + '%' }</p> 
              </Accordion.Toggle>
            </Card.Header>
